...
world.beforeEvents.playerInteractWithBlock.subscribe(e => {
    ...
    if (block.typeId !== "minecraft:noteblock" || !player.isSneaking || !PlayerDataManager.getIsEnable(player) || !PlayerDataManager.getConfig(player, "isReverseEnabled")) return;
    e.cancel = true;
    system.run(async () => {
        const scale = await NoteBlock.getScale(block);
        const instrument = NoteBlock.getInstrument(block);
        if (!scale) return console.error("音階の取得に失敗しました。");
...
system.runInterval(async () => {
    world.getAllPlayers().filter(player => PlayerDataManager.getIsEnable(player)).forEach(player => {
        ...
        (async () => {
            ...
            const scales = Scales[PlayerDataManager.getConfig(player, "scaleDisplayStyle") as ScaleDisplayStyle];

            actionBarMessage += PlayerDataManager.getConfig(player, "language") == "ja" ? "scale: " : "音階: "
            //表示スタイルに応じで取得のやり方を変更
            actionBarMessage += PlayerDataManager.getConfig(player, "scaleDisplayStyle") == "international" ? scales[scaleIndex] : scales[PlayerDataManager.getConfig(player, "language") as string][scaleIndex];
        ...
        if (PlayerDataManager.getConfig(player, "isDisplayInstrument")) actionBarMessage += PlayerDataManager.getConfig(player, "language") == "ja" ? " instrument: " : " 楽器: " + InstrumentsTranslateKey[PlayerDataManager.getConfig(player, "language") as Language][Instruments[instrument]];
