import { CommandPermissionLevel, CustomCommand, CustomCommandOrigin, CustomCommandParamType, CustomCommandResult, CustomCommandSource, CustomCommandStatus, system, Player } from "@minecraft/server";
...
function commandFunc(origin: CustomCommandOrigin, control: string): CustomCommandResult {
    ...
    if (control == "reset") {
        system.run(() => {
            ...
            return { status: CustomCommandStatus.Success, message: playerData.getLang() == "ja" ? "§e設定を初期化しました。" : "§eSettings have been reset." } as CustomCommandResult;
        })
    }
    ...
    if (control == "toggle") {
        system.run(() => {
            ...
            if (playerData.getLang() == "ja") return { status: CustomCommandStatus.Success, message: `${player.getDynamicProperty("isEnable") ? "§e音階表示を有効にしました。" : "§e音階表示を無効にしました。"}` };
            if (playerData.getLang() == "en") return { status: CustomCommandStatus.Success, message: `${player.getDynamicProperty("isEnable") ? "§eThe scale display has been enabled." : "§eThe scale display has been disabled."}`}
        })
    }
