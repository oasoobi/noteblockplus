...
export default class ConfigManager {
    ...
    openConfig() {
        if (PlayerDataManager.getLang(this.#player) == "ja") {
            new ModalFormData()
                .title("設定")
                ...
                .slider("距離", 1, 20, { defaultValue: PlayerDataManager.getConfig(this.#player, "distance"), valueStep: 1, tooltip: "音ブロックの表示距離を設定します。" })
                .toggle("楽器を表示", { defaultValue: PlayerDataManager.getConfig(this.#player, "isDisplayInstrument"), tooltip: "楽器の表示/非表示を切り替えます。重い場合は無効にすると軽くなる場合があります。" })
                .toggle("クリック数を表示", { defaultValue: PlayerDataManager.getConfig(this.#player, "isDisplayClickCount"), tooltip: "クリック数の表示/非表示を切り替えます。" })
                .toggle("しゃがみながら右クリックで音階を一つ下げる", { defaultValue: PlayerDataManager.getConfig(this.#player, "isReverseEnabled"), tooltip: "しゃがみ + 右クリックで音階を一つ下げる機能を有効にします。" })
                ...
                system.run(() => {
                    this.#player.sendMessage(PlayerDataManager.getLang(this.#player) ? "§e設定を変更しました。" : "§eThe settings have been changed.");
                });
            });
        }
        else {
            new ModalFormData()
                ...
                .slider("Distance", 1, 20, { defaultValue: PlayerDataManager.getConfig(this.#player, "distance"), valueStep: 1, tooltip: "Sets the display distance for note blocks." })
                .toggle("Display Instruments", { defaultValue: PlayerDataManager.getConfig(this.#player, "isDisplayInstrument"), tooltip: "Toggle the display of instruments." })
                .toggle("Display Click Count", { defaultValue: PlayerDataManager.getConfig(this.#player, "isDisplayClickCount"), tooltip: "Toggle the display of click count." })
                .toggle("Lower pitch by one step with Sneak + Right Click", { defaultValue: PlayerDataManager.getConfig(this.#player, "isReverseEnabled"), tooltip: "Enables lowering the note pitch by one step when sneaking and right-clicking." })
